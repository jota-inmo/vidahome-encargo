...
const ResidentialForm: React.FC<Props> = ({ formData, handleChange }) => {
...
// Similar components for LandForm, LocalForm, GarageForm, TrasteroForm can be created here
...
export const PropertyDetailsSection: React.FC<Props> = ({ formData, handleChange }) => {
  return (
    <Section title="3) Inmueble">
        ...
        {RESIDENTIAL_TYPES.includes(formData.tipo_vivienda) && <ResidentialForm formData={formData} handleChange={handleChange} />}
        {/* Other forms would be conditionally rendered here */}
    </Section>
  );
};
